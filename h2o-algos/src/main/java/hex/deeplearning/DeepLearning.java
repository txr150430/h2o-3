...
//        if (!mp._quiet_mode) Log.info("Initial model:\n" + model.model_info());
        ...
        for(;;) {
          model.iterations++;
          model.set_model_info(new DeepLearningTask (_job._key,        model.model_info(), rowFraction(train, mp, model), model.iterations).doAll     (    train    ).model_info()); //distributed data (always in multi-node mode)
